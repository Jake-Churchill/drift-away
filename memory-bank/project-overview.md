...
- **Zones extend the grid rather than replacing it** — zone 2 sits at cols 6–11 and is discovered by ordinary adjacency across the col 5/col 6 border, its tiles cost ×15 and produce ×4 what their zone-1 mirrors do, and it resets on prestige like everything else. Fog of war is the existing `isDiscovered` rule plus a purely visual cloud. See `memory-bank/tile-design.md`.
